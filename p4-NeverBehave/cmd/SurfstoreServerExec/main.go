...
func startServer(hostAddr string, serviceType string, blockStoreAddr string) error {
	...
	switch serviceType {
	case "meta":
		ms := surfstore.NewMetaStore(blockStoreAddr)
		surfstore.RegisterMetaStoreServer(grpcServer, ms)
	case "both":
		ms := surfstore.NewMetaStore(hostAddr)
		surfstore.RegisterMetaStoreServer(grpcServer, ms)
		fallthrough
	case "block":
		bs := surfstore.NewBlockStore()
		surfstore.RegisterBlockStoreServer(grpcServer, bs)
	}

	log.Printf("Server started on %v with mode %v", hostAddr, serviceType)
