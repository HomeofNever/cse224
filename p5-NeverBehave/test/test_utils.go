...
func InitSurfServers(blockStores int) []*exec.Cmd {
	...
	if blockStores == 0 {
		serverCmd := exec.Command("_bin/SurfstoreServerExec", "-d", "-s", "both", "-l", "localhost:8080")
		serverCmd.Stderr = os.Stderr
		serverCmd.Stdout = os.Stdout
		cmdList = append(cmdList, serverCmd)
	} else {
		metaArgs := []string{"-s", "meta", "-l", "-d"}
		for i := 1; i <= blockStores; i++ {
			port := 8080 + i
			blockCmd := exec.Command("_bin/SurfstoreServerExec", "-d", "-s", "block", "-p", strconv.Itoa(port), "-l")
