...
func InitBlockStore(blockStorePort string) *exec.Cmd {
	blockCmd := exec.Command("_bin/SurfstoreServerExec", "-d", "-s", "block", "-p", blockStorePort, "-l")
...
func InitRaftServers(cfgPath string) []*exec.Cmd {
	...
	for idx, _ := range cfg {
		cmd := exec.Command("_bin/SurfstoreRaftServerExec", "-d", "-f", cfgPath, "-i", strconv.Itoa(idx), "-b", "localhost:8080")
