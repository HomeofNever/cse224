...
func TestSyncTwice(t *testing.T) {
	t.Logf("client1 syncs with file1. client2 syncs with file1 (different content). client1 syncs again.")
	cfgPath := "./config_files/3nodes.txt"
	test := InitTest(cfgPath, "8080")
	defer EndTest(test)
	...
	if !c {
		t.Fatalf("file1 should not change on both side")
	}
...
// func TestSyncRecoverable(t *testing.T) {
// 	t.Logf("client1 syncs with file1 with crash node, client1 syncs again.")
// 	cfgPath := "./config_files/3nodes.txt"
// 	test := InitTest(cfgPath, "8080")
// 	defer EndTest(test)

// 	leader := 0
// 	test.Clients[leader].SetLeader(test.Context, &emptypb.Empty{})
// 	test.Clients[leader].SendHeartbeat(test.Context, &emptypb.Empty{})
